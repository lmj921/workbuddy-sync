...
> **行业代码**：`media`(传媒) / `elec`(电力设备) / `eltn`(电子) / `re`(房地产) / `text`(纺织服饰) / `nbfin`(非银金融) / `steel`(钢铁) / `utils`(公用事业) / `dfnse`(国防军工) / `env`(环保) / `mach`(机械设备) / `chem`(基础化工) / `comp`(计算机) / `happl`(家用电器) / `bmat`(建筑材料) / `bldg`(建筑装饰) / `trans`(交通运输) / `coal`(煤炭) / `cosm`(美容护理) / `agri`(农林牧渔) / `auto`(汽车) / `trade`(商贸零售) / `socsv`(社会服务) / `petro`(石油石化) / `food`(食品饮料) / `comm`(通信) / `pharm`(医药生物) / `bank`(银行) / `metal`(有色金属)

> **参数说明**：
> - `<行业>`：支持中文名称（如"煤炭"）或英文代码（如"coal"）
...
查询个股事件日历，按事件类型分组输出。宏观经济日历请使用 `macro indicator cn_calendar_future|cn_calendar_hist`。
...
```bash
westock-data news list sh600000 --limit 20     # 新闻列表
...
> `news list` 除个股外，还支持指数（`sh000001`）、ETF（`sh510300`）、板块（`pt01801081`）、可转债（`sh113052`）、期货（`fuCL`）、外汇（`fxUSDCNH`）等代码；接口不分页，用 `--limit` 控制返回条数。
...
# 按日期区间查询（A股/港股/美股均支持，与 --num 互斥）
westock-data finance sh600000 --type lrb --start 2024-01-01 --end 2024-12-31
westock-data finance sh600000,sz000001 --type lrb --start 2024-01-01 --end 2024-12-31
```
...
```bash
westock-data industry-chain list                          # 列出所有产业链主题
westock-data industry-chain                              # 无参数 = list
```
...
```bash
westock-data industry-chain graph 超级电容                   # 查询产业链图谱
westock-data industry-chain graph 超级电容 --category upstream  # 只看上游
westock-data industry-chain graph 白酒 --category midstream   # 只看中游
westock-data industry-chain graph 集成电路 --category downstream # 只看下游
```
...
```bash
westock-data industry-chain sh600519                       # 查询股票所属产业链（简洁风格）
westock-data industry-chain sz000001                       # 平安银行
westock-data industry-chain hk00700                       # 腾讯控股（港股）
```
...
```bash
westock-data industry-chain sh600519,sz000001              # 批量查询多只股票
```
...
| 命令格式 | 功能 | 输出内容 |
|---------|------|----------|
| `industry-chain list` | 列出所有产业链主题 | 主题代码、主题名称、主题类型 |
| `industry-chain graph <主题>` | 查询产业链图谱 | 上下游分类、节点、相关股票 |
| `industry-chain <股票代码>` | 查询股票所属产业链 | 主题列表、节点列表、关联度 |

**使用场景**：
- 泛问"有哪些产业链" → `industry-chain list`
- 问"白酒产业链的上下游分布" → `industry-chain graph 白酒`
- 问"贵州茅台属于哪些产业链" → `industry-chain sh600519`
- 问"只看新能源汽车产业链的上游环节" → `industry-chain graph 新能源汽车 --category upstream`
