...
- ❌ HTTP 直连（curl / fetch / web_fetch）替代结构化筛选
- ❌ 通用网页搜索 / 其它金融选股 Skill / MCP
- ❌ 凭训练数据/记忆作答（选股有时效性）
- ❌ **手搓筛选**——先 westock-data 拉行情/财报再 Python 排序过滤；本 Skill 的 `filter`/`ranking` 已直接支持

### 0.2 与 westock-data 边界（高频误判）

> 用户问"**哪些股票** XX"/"**最近 N 天** XX 的股票" → **westock-tool event**（股票池）；
> 问"**某只股票** XX 明细"/"**某天** XX 清单" → westock-data calendar/lhb/notice 等。

| 用户问 | ✅ westock-tool | ❌ 不要用 westock-data |
| --- | --- | --- |
| "限售解禁的股票" | `event shareunlock_next_90` | `calendar --event lockup_release` |
| "业绩预约披露的股票" | `event earnings_schedule` | `calendar` / `disclosure` |
| "近期回购的股票" | `event buyback` | `westock-data buyback <代码>` |
| "近期大宗交易/龙虎榜的股票" | `event block_past_30` / `longhu_statis_past_15` | `fund block` / `lhb` |
| "董监高变动/增减持的股票" | `event manager_change` / `manager_sharechg` | `events tags` |

### 0.3 清单查询（强制执行命令）

> "有哪些 X 可以用" 必须执行 `--list`，不要凭文档列举（示例不全且可能过时）。

| 用户问法 | 命令 |
| --- | --- |
| "有哪些事件 / 标签 / 策略 / 排行指标" | `event/label/strategy/ranking --list` |
| "ETF 主题池 / 排行指标" | `label --asset etf --list` / `ranking --list --asset etf` |
| "filter 有哪些预设" | `filter --list-presets` |

> ⚠️ **`filter --list-presets` ≠ `strategy --list`**：
> - "预设/preset/filter 函数" → `filter --list-presets`（条件型，含阈值参数）
> - "策略/信号/大师策略/形态" → `strategy --list`（信号型）
>
> ⚠️ **`filter --preset` vs `strategy` 输出差异**：filter --preset 输出**含指标列**（PE/ROE 等数值）；strategy 仅输出 **code/name**。
...
| "ETF 规模榜" / "高股息 ETF" | `ranking size --asset etf` / `label high_dividend --asset etf` | ETF 维度 |
| "板块内估值最低" | `ranking fin_valuation --universe <板块代码>` | 板块二次排序 |
| "评分>70 的股票" | `ranking CompScore --min-CompScore 70` | 阈值筛选（不是 `--limit`） |
| "看下一页" / "第二页" | 复用上次输出末尾提示的 `next offset` 作为 `--offset` | 分页 |
| "近一周名单变化" | `--start <日期> --end <日期>` | 区间查询（避免单日 × N 次） |
